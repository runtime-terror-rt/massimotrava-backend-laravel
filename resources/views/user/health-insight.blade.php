...
@section('page_title_key', 'sb_insights')
...
.hi-title{font-family:'Syne',sans-serif;font-size:24px;font-weight:800;letter-spacing:-0.03em;color:var(--text)}
.hi-sub{font-size:13px;color:var(--text-muted);margin-top:3px}
...
/* ── Alert banner ── */
.alert-banner{background:rgba(34,211,238,0.04);border:1px solid rgba(34,211,238,0.18);border-radius:14px;padding:13px 20px;display:flex;align-items:center;gap:10px;font-size:13px;color:var(--text-muted);margin-bottom:24px}
...
.sc-big{font-family:'Syne',sans-serif;font-size:56px;font-weight:900;color:var(--text);line-height:1}
.sc-denom{font-size:16px;color:var(--text-muted);margin-bottom:8px}
.sc-meta-row{display:flex;align-items:center;gap:10px;margin-top:6px;flex-wrap:wrap}
.sc-since{font-size:11.5px;color:var(--text-muted)}
...
.sc-message{font-size:12.5px;color:var(--text-muted);line-height:1.65;font-style:italic;border-top:1px solid var(--border);padding-top:14px}
.sc-kit-meta{font-size:11px;color:var(--text-muted);margin-top:8px}

/* ── Summary card ── */
.summary-card{background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:24px 26px;display:flex;flex-direction:column;gap:18px}
.summary-headline{font-size:15.5px;line-height:1.75;color:var(--text)}
...
.focus-tag-sub{font-size:11.5px;color:var(--text-muted)}

/* ── Marker rows ── */
.marker-row{background:var(--surface-2);border:1px solid var(--border);border-radius:14px;padding:14px 18px;display:flex;align-items:center;gap:14px;transition:border-color .2s}
.marker-row:hover{border-color:var(--accent)}
...
.marker-name{font-size:13.5px;font-weight:700;color:var(--text);margin-bottom:2px}
.marker-status{font-size:11.5px;color:var(--text-muted)}
...
.sec-title{font-family:'Syne',sans-serif;font-size:16px;font-weight:800;color:var(--text)}

/* ── Smart Insights ── */
.insights-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:22px}
.insight-card{background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:20px 22px;transition:border-color .2s,transform .2s}
...
.ic-title{font-size:13.5px;font-weight:700;color:var(--text);line-height:1.45;flex:1}
.ic-body{font-size:12.5px;color:var(--text-muted);line-height:1.65}
...
.trend-card{background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:16px 18px;transition:border-color .2s}
...
.tc-name{font-size:13px;font-weight:700;color:var(--text);margin-bottom:2px}
.tc-sub{font-size:11.5px;color:var(--text-muted);margin-bottom:8px}
...
.rt-title{font-size:16px;font-weight:700;color:var(--text);margin-bottom:6px;line-height:1.4}
.rt-body{font-size:13px;color:var(--text-muted);line-height:1.65;max-width:580px}
...
<div class="hi">
    ...
    <div class="hi-topbar">
        <div>
            <div class="hi-title">{{ __('messages.title') }}</div>
            <div class="hi-sub">{{ __('messages.sub_title') }}</div>
        </div>
        <div class="analysis-badge">
            <span></span> {{ __('messages.analysis_complete') }}
        </div>
    </div>
    ...
    <div class="alert-banner">
        ...
        <span><b>{{ __('messages.update_label') }}:</b> {{ __('messages.alert_received_text') }}</span>
    </div>
    ...
    <div class="hi-main-grid">
        ...
        <div class="score-card">
            <div class="sc-label">{{ __('messages.longevity_score') }}</div>
            <div class="sc-num-row">
                <div class="sc-big">{{ $longevityScore ?? 84 }}</div>
                <div class="sc-denom">/100</div>
            </div>
            <div class="sc-meta-row">
                <span class="sc-since">{{ __('messages.since') }} <b>{{ $scoreSince ?? 'Oct 2025' }}</b></span>
                <span class="sc-improve">↑ +{{ $scoreImprovement ?? 12 }} {{ __('messages.improvement') }}</span>
            </div>
            ...
            <div class="mini-chart">
                <svg viewBox="0 0 280 72" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <line x1="0" y1="20" x2="280" y2="20" stroke="var(--border)" stroke-width="1"/>
                    <line x1="0" y1="40" x2="280" y2="40" stroke="var(--border)" stroke-width="1"/>
                    ...
                    <circle cx="280" cy="14" r="4" fill="#10b981" stroke="var(--surface)" stroke-width="2"/>
                    <text x="2" y="70" font-size="9" fill="var(--text-muted)" font-family="Inter">{{ __('messages.month_oct') }}</text>
                    <text x="43" y="70" font-size="9" fill="var(--text-muted)" font-family="Inter">{{ __('messages.month_nov') }}</text>
                    <text x="90" y="70" font-size="9" fill="var(--text-muted)" font-family="Inter">{{ __('messages.month_dec') }}</text>
                    <text x="137" y="70" font-size="9" fill="var(--text-muted)" font-family="Inter">{{ __('messages.month_jan') }}</text>
                    <text x="184" y="70" font-size="9" fill="var(--text-muted)" font-family="Inter">{{ __('messages.month_feb') }}</text>
                    <text x="231" y="70" font-size="9" fill="var(--text-muted)" font-family="Inter">{{ __('messages.month_mar') }}</text>
                    <text x="236" y="12" font-size="9" fill="#10b981" font-family="Inter" font-weight="700">{{ __('messages.chart_now', ['score' => $longevityScore ?? 84]) }}</text>
                    <text x="2" y="56" font-size="9" fill="var(--text-muted)" font-family="Inter">{{ __('messages.chart_prev', ['score' => $previousScore ?? 74]) }}</text>
                </svg>
            </div>

            <div class="sc-message">"{{ __('messages.score_message') }}"</div>
            <div class="sc-kit-meta">
                {{ __('messages.kit_meta_text', ['date' => $testDate ?? 'Mar 25, 2026', 'kit' => $kitNumber ?? '23281', 'ago' => $updatedAgo ?? '2 days ago']) }}
            </div>
        </div>
        ...
        <div class="summary-card">
            <div class="summary-headline">
                {{ __('messages.summary_headline_start') }} <span class="imp">{{ __('messages.summary_headline_imp') }}</span>{{ __('messages.summary_headline_mid') }}
                <span class="warn">{{ __('messages.summary_headline_warn') }}</span>
            </div>
            ...
            <div>
                <div class="focus-tag">
                    ...
                    <span class="focus-tag-label" style="color:#f87171">{{ __('messages.primary_focus') }}</span>
                    <span class="focus-tag-sub">{{ __('messages.compare_baseline_text') }}</span>
                </div>
                <div class="marker-row">
                    <div class="marker-icon mi-red">🫀</div>
                    <div class="marker-info">
                        <div class="marker-name">{{ __('messages.biomarker_ldl') }}</div>
                        <div class="marker-status">{{ __('messages.status_needs_attention') }}</div>
                    </div>
                    <div class="marker-right">
                        <span class="priority-badge pb-high">{{ __('messages.priority_high') }}</span>
                        <span class="change-badge cb-up">+8% {{ __('messages.badge_increase') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="focus-tag">
                    <span style="font-size:16px">📈</span>
                    <span class="focus-tag-label" style="color:#10b981">{{ __('messages.improving') }}</span>
                    <span class="focus-tag-sub">{{ __('messages.compare_baseline_text') }}</span>
                </div>
                <div class="marker-row">
                    <div class="marker-icon mi-green">☀️</div>
                    <div class="marker-info">
                        <div class="marker-name">{{ __('messages.biomarker_vit_d') }}</div>
                        <div class="marker-status">{{ __('messages.improving') }}</div>
                    </div>
                    <div class="marker-right">
                        <span class="priority-badge pb-good">{{ __('messages.priority_good') }}</span>
                        <span class="change-badge cb-down">+4% {{ __('messages.badge_increase') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="focus-tag">
                    <span style="font-size:16px">⚡</span>
                    <span class="focus-tag-label" style="color:#22d3ee">{{ __('messages.stable_monitor') }}</span>
                    <span class="focus-tag-sub">{{ __('messages.compare_baseline_text') }}</span>
                </div>
                <div class="marker-row">
                    <div class="marker-icon mi-blue">🧬</div>
                    <div class="marker-info">
                        <div class="marker-name">{{ __('messages.biomarker_testosterone') }}</div>
                        <div class="marker-status">{{ __('messages.status_stable') }}</div>
                    </div>
                    <div class="marker-right">
                        <span class="priority-badge pb-stable">{{ __('messages.priority_stable') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    {{-- ── Smart Insights ── --}}
    <div class="sec-head">
        <div class="sec-title">💡 {{ __('messages.section_smart_insights') }}</div>
    </div>
    <div class="insights-grid">

        <div class="insight-card priority">
            <div class="ic-top">
                <div class="ic-title">{{ __('messages.smart_ldl_title_1') }}</div>
                <span class="priority-badge pb-high">{{ __('messages.priority_high') }}</span>
            </div>
            <div class="ic-body">{{ __('messages.smart_ldl_body_1') }}</div>
            <div class="ic-impact">
                <div class="ic-impact-dot"></div>
                {{ __('messages.impact_cardio_longevity') }}
            </div>
        </div>

        <div class="insight-card priority">
            <div class="ic-top">
                <div class="ic-title">{{ __('messages.smart_ldl_title_2') }}</div>
                <span class="priority-badge pb-high">{{ __('messages.priority_high') }}</span>
            </div>
            <div class="ic-body">{{ __('messages.smart_ldl_body_2') }}</div>
            <div class="ic-impact">
                <div class="ic-impact-dot"></div>
                {{ __('messages.impact_cardio_longevity') }}
            </div>
        ...
        <div class="insight-card">
            <div class="ic-top">
                <div class="ic-title">{{ __('messages.smart_vit_d_title') }}</div>
                <span class="priority-badge pb-good">{{ __('messages.priority_good') }}</span>
            </div>
            <div class="ic-body">{{ __('messages.smart_vit_d_body') }}</div>
            <div class="ic-impact">
                <div class="ic-impact-dot"></div>
                {{ __('messages.impact_immune_bone') }}
            </div>
        </div>

        <div class="insight-card">
            <div class="ic-top">
                <div class="ic-title">{{ __('messages.smart_testosterone_title') }}</div>
                <span class="priority-badge pb-stable">{{ __('messages.priority_stable') }}</span>
            </div>
            <div class="ic-body">{{ __('messages.smart_testosterone_body') }}</div>
            <div class="ic-impact">
                <div class="ic-impact-dot"></div>
                {{ __('messages.impact_energy_metabolism') }}
            </div>
        </div>
    ...
    {{-- ── Biomarker Trends ── --}}
    <div class="sec-head">
        <div class="sec-title">📊 {{ __('messages.section_biomarker_trends') }}</div>
    </div>
    <div class="trends-grid">

        <div class="trend-card tc-low">
            <div class="tc-badge tcb-low">● {{ __('messages.badge_low') }}</div>
            <div class="tc-name">{{ __('messages.biomarker_vit_d') }}</div>
            <div class="tc-sub">{{ __('messages.improving') }}</div>
        ...
        <div class="trend-card tc-optimal">
            <div class="tc-badge tcb-opt">● {{ __('messages.badge_optimal') }}</div>
            <div class="tc-name">{{ __('messages.biomarker_cortisol') }}</div>
            <div class="tc-sub">{{ __('messages.improving') }}</div>
        ...
        <div class="trend-card tc-low">
            <div class="tc-badge tcb-low">● {{ __('messages.status_needs_attention') }}</div>
            <div class="tc-name">{{ __('messages.biomarker_ldl') }}</div>
            <div class="tc-sub">{{ __('messages.trend_rising') }}</div>
        ...
        <div class="trend-card tc-optimal">
            <div class="tc-badge tcb-opt">● {{ __('messages.badge_optimal') }}</div>
            <div class="tc-name">{{ __('messages.biomarker_testosterone') }}</div>
            <div class="tc-sub">{{ __('messages.status_stable') }}</div>
            <div class="tc-pct" style="color:#10b981">→ {{ __('messages.status_stable') }}</div>
            <div class="tc-chart">
                <svg viewBox="0 0 120 42" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="retest-card">
        <div>
            <div class="rt-eyebrow">
                <i class="fa-regular fa-calendar-check"></i> {{ __('messages.retest_reminder_title') }}
            </div>
            <div class="rt-title">{{ __('messages.retest_headline') }}</div>
            <div class="rt-body">{{ __('messages.retest_body_text') }}</div>
        </div>
        {{-- <button class="rt-btn" onclick="sendRetestMessage()">
            <i class="fa-solid fa-paper-plane" style="margin-right:6px"></i> {{ __('messages.btn_send_message') }}
        </button> --}}
    </div>
